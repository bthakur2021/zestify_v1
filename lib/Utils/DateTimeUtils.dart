...
class DateTimeUtils {
 ...
 /* int getDateTimeInMilliseconds(String createdDateTime){
    var notiCreatedDateTime = DateTime.parse(createdDateTime);
    return notiCreatedDateTime.millisecondsSinceEpoch;
  }
*/

  String formatTime(String createdDateTime) {

    int timeStamp = DateTime.parse(createdDateTime).millisecondsSinceEpoch;
  ...
  String countSeconds(int difference) {
    ...
    return count > 1 ? count.toString() + ' s' : 'Just now';
  }
  ...
  String countMinutes(int difference) {
    ...
    return count.toString() + (count > 1 ? ' m' : ' m');
  }
  ...
  String countHours(int difference) {
    ...
    return count.toString() + (count > 1 ? ' h' : ' h');
  }
  ...
  String countDays(int difference) {
    ...
    return count.toString() + (count > 1 ? ' d' : ' d');
  }
  ...
  String countWeeks(int difference) {
    ...
    return count.toString() + (count > 1 ? ' w' : ' w');
  }
  ...
  String countMonths(int difference) {
    ...
    return count.toString() + (count > 1 ? ' mo' : ' mo');
  }
  ...
  String countYears(int difference) {
    ...
    return count.toString() + (count > 1 ? ' y' : ' y');
  }
}